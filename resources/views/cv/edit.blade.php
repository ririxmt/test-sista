@php
    $pendidikan = $cv->pendidikanFormal->map(fn ($p) => [
        'jenjang' => $p->tingkat, 'institusi' => $p->institusi, 'jurusan' => $p->jurusan, 'tahun_lulus' => $p->tahun_lulus,
    ])->toArray();
    $pelatihan = $cv->pendidikanNonformal->map(fn ($p) => [
        'nama_pelatihan' => $p->nama_pelatihan, 'penyelenggara' => $p->penyelenggara, 'tahun' => $p->tahun,
    ])->toArray();
    $pengalaman = $cv->pengalamanKerja->map(fn ($p) => [
        'nama_kegiatan' => $p->nama_kegiatan, 'pemberi_pekerjaan' => $p->pemberi_pekerjaan, 'posisi' => $p->posisi,
        'tahun' => $p->tahun, 'lokasi' => $p->lokasi, 'durasi' => $p->durasi, 'deskripsi' => $p->uraian_tugas,
    ])->toArray();
    $sertifikasi = $cv->sertifikasiProfesi->map(fn ($s) => [
        'id' => $s->id, 'nama_sertifikasi' => $s->nama, 'penerbit' => $s->penerbit, 'tahun' => $s->tahun,
        'file_sertifikat' => $s->file_sertifikat,
    ])->toArray();
    $proyek = $cv->projectRelevan->map(fn ($p) => [
        'nama_proyek' => $p->nama_project, 'klien' => $p->klien, 'tahun' => $p->tahun,
    ])->toArray();
    $bahasa = $cv->bahasa->map(fn ($b) => [
        'bahasa' => $b->bahasa, 'speaking' => $b->speaking, 'reading' => $b->reading, 'writing' => $b->writing,
    ])->toArray();
    $skills = $cv->cvKeahlian->pluck('nama_keahlian')->toArray();

    $d = [
        'gelar_depan'      => $cv->gelar_depan,
        'nama'             => $cv->nama,
        'gelar_belakang'   => $cv->gelar_belakang,
        'tempat_lahir'     => $cv->tempat_lahir,
        'tanggal_lahir'    => $cv->tanggal_lahir?->format('Y-m-d'),
        'posisi'           => $cv->posisi,
        'perusahaan'       => $cv->perusahaan,
        'kewarganegaraan'  => $cv->kewarganegaraan,
        'domisili_kota'    => $cv->domisili_kota,
        'domisili_negara'  => $cv->domisili_negara,
        'ringkasan_profil' => $cv->employment_desc,
    ];
@endphp
<!DOCTYPE html>
<html lang="id">
...
<body>
    <a href="{{ route('cv.show', $cv) }}" class="back">&larr; Kembali ke detail CV</a>

    <h1>Edit CV</h1>
    <p class="muted">Ubah field mana pun di bawah, lalu klik <strong>Simpan Perubahan</strong>.</p>
    ...
    <form action="{{ route('cv.update', $cv) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="card">
            ...
            <div class="grid2">
                ...
                <div>
                    ...
                    <input type="text" name="kewarganegaraan" value="{{ old('kewarganegaraan', $d['kewarganegaraan']) }}">
                </div>
                <div>
        ...
        <div class="card">
            ...
            <div data-list="pengalaman_kerja">
                @foreach ($pengalaman as $i => $p)
                    <div class="row">
                        <button type="button" class="btn btn-remove" data-remove style="position:absolute;top:12px;right:12px;">Hapus</button>
                        <div><label>Nama Kegiatan / Proyek</label><input type="text" name="pengalaman_kerja[{{ $i }}][nama_kegiatan]" value="{{ $p['nama_kegiatan'] ?? '' }}"></div>
                        <div class="grid3">
                            <div><label>Pemberi Pekerjaan</label><input type="text" name="pengalaman_kerja[{{ $i }}][pemberi_pekerjaan]" value="{{ $p['pemberi_pekerjaan'] ?? '' }}"></div>
                            <div><label>Posisi</label><input type="text" name="pengalaman_kerja[{{ $i }}][posisi]" value="{{ $p['posisi'] ?? '' }}"></div>
            ...
            <div data-list="sertifikasi">
                @foreach ($sertifikasi as $i => $s)
                    <div class="row">
                        ...
                        {{-- bawa path file lama agar tidak hilang saat disimpan ulang --}}
                        <input type="hidden" name="sertifikasi[{{ $i }}][file_sertifikat]" value="{{ $s['file_sertifikat'] ?? '' }}">
                        @if (!empty($s['file_sertifikat']) && !empty($s['id']))
                            <p class="muted" style="margin:6px 0 2px;">
                                File saat ini: <a href="{{ route('cv.sertifikat-file', $s['id']) }}" target="_blank">lihat file &nearr;</a>
                                <label style="font-weight:400;color:#b91c1c;margin-left:8px;">
                                    <input type="checkbox" name="sertifikasi_hapus_file[{{ $i }}]" value="1" style="width:auto;"> hapus file
                                </label>
                            </p>
                        @endif
                        <label>Ganti / Tambah File Sertifikat (PDF/JPG/DOC/DOCX)</label>
        ...
        @php
            ...
            $kolomLampiran = ['ktp' => 'ktp_file', 'npwp' => 'npwp_file', 'bukti_pajak' => 'bukti_pajak', 'foto' => 'foto'];
            $ijazahList = $cv->lampiran?->lainnya['ijazah'] ?? [];
        @endphp
        <div class="card">
            <div class="card-head"><h2>Lampiran Dokumen (PDF / JPG / JPEG)</h2></div>
            <p class="muted" style="margin:0 0 8px;">Unggah untuk menambah/mengganti (PDF/JPG/DOC/DOCX, maks. 50MB per file). File baru menggantikan yang lama.</p>
            <div class="grid2">
                @foreach ($jenisLampiran as $key => $label)
                    @php $ada = $cv->lampiran?->{$kolomLampiran[$key]}; @endphp
                    <div class="row" style="background:#fff;">
                        <label style="margin-top:0;">{{ $label }}</label>
                        @if ($ada)
                            <p class="muted" style="margin:0 0 6px;">
                                File saat ini:
                                <a href="{{ route('cv.lampiran', [$cv, $key]) }}" target="_blank">lihat file &nearr;</a>
                            </p>
                            <label style="font-weight:400;color:#b91c1c;">
                                <input type="checkbox" name="lampiran_hapus[{{ $key }}]" value="1" style="width:auto;"> Hapus file ini
                            </label>
                        @else
                            <p class="muted" style="margin:0 0 6px;">Belum ada file.</p>
                        @endif
                        <input type="file" name="lampiran[{{ $key }}]" accept=".pdf,.jpg,.jpeg,.png,.doc,.docx,application/pdf,image/jpeg,image/png,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document">
                    </div>
                @endforeach
            </div>

            {{-- Ijazah: bisa banyak file (Sertifikat diunggah per baris di bagian Sertifikasi Profesi) --}}
            <div class="row" style="background:#fff;margin-top:12px;">
                <label style="margin-top:0;">Ijazah (bisa banyak file)</label>
                @forelse ($ijazahList as $i => $path)
                    <div style="margin:2px 0;">
                        <a href="{{ route('cv.lampiran-multi', [$cv, 'ijazah', $i]) }}" target="_blank">Ijazah #{{ $i + 1 }} &nearr;</a>
                        <label style="font-weight:400;color:#b91c1c;margin-left:8px;">
                            <input type="checkbox" name="ijazah_hapus[]" value="{{ $i }}" style="width:auto;"> hapus
                        </label>
                    </div>
                @empty
                    <p class="muted" style="margin:0 0 6px;">Belum ada file.</p>
                @endforelse
                <label style="margin-top:8px;">Tambah file Ijazah:</label>
                <input type="file" name="ijazah_files[]" accept=".pdf,.jpg,.jpeg,.png,.doc,.docx,application/pdf,image/jpeg,image/png,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document" multiple>
            </div>
        </div>

        <div class="actions">
            <button type="submit" class="btn btn-primary">Simpan Perubahan</button>
            <a href="{{ route('cv.show', $cv) }}" class="muted">Batal</a>
        </div>
    </form>
